...
ggpairs(county_num_df[,c(1:half, end-1)])

ggpairs(county_num_df[,half:end-1])
...
x_matrix = county_num_df %>% #how is county_class being treated in regularization techniques
  select(-YPLL) %>% 
  as.matrix()
...
init_reg_fit <- lm(YPLL ~ ., county_num_df)
...
```{r}
#group lasso (correlation within group structure of features e.g., diet/exercise)
```
...
lm_mod = lm(YPLL ~ std + smoke, data = county_tobacco_sex_df)
#lm_mod = lm(YPLL ~ std + smoke*county_class, data = county_tobacco_sex_df)
#lm_mod = lm(YPLL ~ std + smoke + log(Population), data = county_tobacco_sex_df)

autoplot(lm_mod, ncol = 2) + theme_bw() #one or two outliers but otherwise pretty good

#Output
summary(lm_mod) #this is actually pretty good and then x's seem to be independent of each other
```
...
mixed_mod = lmer(YPLL ~ smoke + std + (1 | county_class), data = county_tobacco_sex_df)
...
```{r}
#check cv lecture to make cv func
#stratified k fold
set.seed(1)

state_list = unique(county_tobacco_sex_df$state)
strat_df = data.frame()

for (i in 1:length(state_list)){
  df = filter(county_tobacco_sex_df, state == state_list[i])
  new_df = mutate(df, fold = sample(1:5, replace = T, size = nrow(df)))
