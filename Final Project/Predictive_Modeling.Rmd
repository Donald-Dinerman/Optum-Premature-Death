...
#Note use fct_recode to change labels easy in dplyr
#e.g., fct_recode(next_score_type, "-Field Goal (-3)" = "Opp_Field_Goal")
```
...
#HELPPP: I tested models using df w p parameters and the df was filtered w na.omit on all p. But i settled on a model with p-4 parameters, so can I refilter my dataframe so it only na.omit on those p-4 parameters? (difference of ~80 obs)? How can I do variable importance plot with gam?
...
# To Do: (1) Shiny App, (2) Formalize EDA, Methods and Conclusions, (3) Publish Shiny 
