...
#df_names = colnames(strat_cv_df) #need to match data input and colnames
```
...
server = function(input,output){
  ...
  shiny_df <- reactive({
    new_table = data.frame((matrix(nrow = 1, data = x_value())))
  ...
  #Get predictions
  shiny_pred <- reactive({
    predict(final_gam_model, newdata = shiny_df()) #test prediction syntax for gam
  })
