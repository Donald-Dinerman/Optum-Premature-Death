...
lm_mod = lm(YPLL ~ std + smoke + food_per, data = county_num_df)
...
#### HELP: Add mixed effects model (why is it the same as lm), explore/discuss CV results, discuss alternative linear modeling approaches, should I explore alternative tree based approaches (can look into xgboost[regularization approach built in])
```{r}
#stratified k fold
set.seed(1)
...
for (i in 1:length(state_list)){
  ...
  new_df = mutate(df, test_fold = sample(1:5, replace = T, size = nrow(df)))
  strat_df = rbind(strat_df, new_df)
}

#note: reg techniques don't accept char x, county_class doesn't help predictions anyways
strat_cv_df = strat_df %>% ungroup() %>% select(-c(state, County)) 
...
holdout_predictions <- 
  map_dfr(unique(strat_cv_df$test_fold), 
          ...
          function(holdout) {
            ...
            # Train models:
            lm_model <- lm(YPLL ~ ., data = train_data)
            mixed_model <- lmer(YPLL ~ . + (1 | county_class), data = train_data)
            # ridge_model <- cv.glmnet(train_x, train_data$YPLL, alpha = 0)
            # lasso_model <- cv.glmnet(train_x, train_data$YPLL, alpha = 1)
            # en_model <- cv.glmnet(train_x, train_data$YPLL, alpha = .5)
            gbm_model <- gbm(train_data$YPLL ~.,
                data = train_data,
                distribution = "gaussian",
                cv.folds = 1, #cv within cv?, is the gbm selecting trees using cv within the train data
                shrinkage = .1,
                n.minobsinnode = 10,
                n.trees = 250,
                verbose = F)
            ...
            # Return tibble of holdout results:
            tibble(lm_preds = predict(lm_model, newdata = test_data), #linear model
                   mixed_preds = predict(mixed_model, newdata = test_data, re.form = ~(1 | county_class)), #mixed model
                   # ridge_preds = as.numeric(predict(ridge_model, newx = test_x)), #ridge
                   # lasso_preds = as.numeric(predict(lasso_model, newx = test_x)), #lasso
                   # en_preds = as.numeric(predict(en_model, newx = test_x)), #elastic net
                   gbm_preds = predict.gbm(gbm_model, newdata = test_data), #gbm model
                   test_actual = test_data$YPLL, test_fold = holdout)
          })

holdout_predictions %>%
  pivot_longer(lm_preds:gbm_preds, 
               names_to = "type", values_to = "test_preds") %>%
  group_by(type, test_fold) %>%
  summarize(rmse = sqrt(mean((test_actual - test_preds)^2))) %>% 
  ggplot(aes(x = type, y = rmse)) + 
...
The best performing models are GBM and linear regression. May go with linear regression which makes sense because the modeled y ~ x relationships are linear but I want a little fancier type of linear regression.
