...
demo_map = function(df = demo_response_clean_df, Race_Type, limit_set = c()) {
  ...
  plot_usmap(data = filter(df, Race == filter_input), values = "YPLL", color = "black") + 
...
demo_map(Race_Type = "Asian")
demo_map(Race_Type = "Native American")
demo_map(Race_Type = "Black")
demo_map(Race_Type = "Hispanic")
demo_map(Race_Type = "White")
demo_map(Race_Type = "All", limit_set = c(5000,11500))
```
...
## Can TEST county_class vs population variable (this can output per capita YPLL)
...
```{r}
#stratified k fold
strat_cv = function(k = 5, seed = 100){
...
Simple gam (7 features) performs approximately the same as gam with all 11 features. For simplicity in potential Shiny App interface simple gam seems to be the way to go.

```{r}
library(hablar)

#Finalized model
final_gam_model <- gam(YPLL ~ s(inactive_per) + s(obesity_rate) + s(food_per) + s(drink_per) + s(smoke) + s(std) + as.factor(county_class),
                      data = county_num_df, 
                      method = "REML")
...
# To Do: (1) Get year labels on all features, (2) Shiny App (Modify functional EDA), (3) Formalize EDA, Methods and Conclusions 
#QUESTION: When doing CV/model validation, do we just use it to say "this is the best performing model" then fit the model on all of our sample data?
# Shiny App
  ...
  src = "https://512solutions.com/wp-content/uploads/2021/04/Optum-logo.png", height = 55, width = 190), ""),
          ...
          tags$head(
           tags$style(HTML('.navbar-nav > li > a, .navbar-brand {
                            ...
                            height: 65px; #Alter height of navbarpage using CSS
                            }'))
         ),
      ...
      tabPanel("Predictive Modeling",
           ...
           fluidPage(
             ...
             titlePanel(title = h4("Premature Deaths Prediction", align = "center")),
    ...
    tabPanel("Premature Deaths Map",
             
    fluidPage(
        
        titlePanel(title = h4("Premature Deaths Map", align = "center")),
...
server = function(input,output){
  ...
  pred_reactive <- eventReactive(input$go, #put the expected YPLL text together
    {
      paste("The expected county YPLL is:", shiny_pred(), "Years")
  })
