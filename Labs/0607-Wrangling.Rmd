...
```
The summary() function provides the summary statistics (e.g., mean, median, max, min, etc.) for each of the variables/columns in the tibble.

The str() function, on the other hand, provides the dimensions of the tibble and displays a single line with the first few entries and class type for each variable/column in the tibble.
```
...
We can also select the rows/observations that satisfy certain criteria. Try selecting the rows with more than 500 duration:

```{r}
colnames(heart_disease)

#filter(heart_disease, Duration > 500) #ignore
```

We can also filter on mutiple criteria. Select rows with `Age` above 60 and the `gender` is 'Male':

```{r}
filter(heart_disease, Age > 60, Gender == "Male")
```
