...
```
Quantitaive: Cost, Age, Interventions, ERVisit, Comorbidities, Duration, id, cost_per_day
...
```
we have to specify `as.factor(Drugs)` first then place the numbers in quotation marks because they should be read in as characters rather than numeric.
```
...
```
blank1 should be a data frame and blank2 should be a variable in the dataframe.
```

+ What do you think the line `geom_bar(fill = "darkblue")` does?

```
Creates bar chart and colors it in with darkblue.
```

+ What do you think the remaining lines of code do (contained in `labs()`)?

```
Apply name labels to the title and axis.
```
...
heart_disease %>%
  ...
  labs(title = "Number of patients colored by number of drugs",
...
```
Instead of the default continuous scale, this function puts the y variable (Number of patients) on a square root scale. When making the rose diagram, this function is useful because it makes the petals' area directly proportional to the number of patients instead of proportional to the number of patients squared.
```
...
```
alpha changes the boldness of the colors for the points in the scatterplot.
```
...
```
I prefer the new color pallette because the colors are distinct but none of them overly stands out, overshadowing the other ones.
```
...
```
This reveals that patients with low costs typically have low interventions and patients with higher costs typically have higher interventions.

If I delete `scale_color_viridis_c() +` from above the color of the points deafults to the blue shaded scale. I prefer the scale_color_viridis_c() color because it is easier to distinguish lower intervention levels from higher ones.
```
# CONTINUE HERE 
## Notes on themes
...
my_theme <- function () {
  ...
  theme_bw(base_size = 10) %+replace% #What's the difference between '%+replace%' and '+'
    theme(
