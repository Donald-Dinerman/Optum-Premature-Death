...
library(tidyverse)
library(magick)
image_read("http://www.stat.cmu.edu/~pfreeman/Figure_4.2.png") %>% print()
```
...
The odds of making a kick decrease by 10% (e^{-0.104365} = 0.90, since 0.90 < 1 this represents a decrease that is odds * 90% which is equivalent to a 1-0.9 decrease in odds) for every additional yard.
...
corresponds
  - Null deviance  to intercept-only model
...
#Can see that since geom smooth line deviates from diagnol, that the model performs best for high prob outcomes and worse for low prob outcomes
nfl_fg_attempts %>%
  mutate(pred_prob = init_logit$fitted.values,
         bin_pred_prob = round(pred_prob / 0.05) * .05) %>% #to round by 0.05 level
  # Group by bin_pred_prob:
  group_by(bin_pred_prob) %>%
  # Calculate the calibration results:
  summarize(n_attempts = n(),
            bin_actual_prob = mean(is_fg_made),
            min_dist = min(kick_distance),
            max_dist = max(kick_distance)) %>%
