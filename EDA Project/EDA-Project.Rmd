...
maternity_df %>% select(State:Births) %>% head() %>% gt()
```
...
```{r}
test_df = filter(maternity_df, PriorBirthsNowDeceased != "Unknown", TobaccoUse != "Unknown") %>%
  mutate(new_dead_baby = ifelse(PriorBirthsNowDeceased == "0", "None", "At least One"))

mosaicplot(table(test_df$new_dead_baby, test_df$TobaccoUse),
           shade = T,
           main = "Tobacco Use Higher Than Expected for mothers with at least 1 prior birth deceased ", ylab = "Tobacco Use")
```
